...
url: watermark/net/mcp/windows-installation
title: Install on Windows
weight: 2
description: "To install the GroupDocs.Watermark MCP server on Windows, run the prerequisite bootstrapper and the guided installer in PowerShell — it registers the server in Claude Desktop, VS Code, and other clients automatically."
...
To install the GroupDocs.Watermark MCP server on **Windows**, run the prerequisite bootstrapper and the guided installer in PowerShell — it registers the server in Claude Desktop, VS Code, and other clients automatically:
...
```powershell
./install-groupdocs-mcp.ps1 -Channel nuget -Products watermark `
...
Client config locations on Windows: Claude Desktop `%APPDATA%\Claude\claude_desktop_config.json`, VS Code user-level `%APPDATA%\Code\User\mcp.json` — full list with JSON blocks in [Register in AI clients]({{< ref "watermark/net/mcp/install-in-ai-clients.md" >}}).
...
Installing on a different OS? [Linux]({{< ref "watermark/net/mcp/linux-installation.md" >}}) · [macOS]({{< ref "watermark/net/mcp/macos-installation.md" >}})
